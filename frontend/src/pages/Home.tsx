...
export const Home: React.FC = () => {
  ...
  const [nlGeneratedExplanation, setNlGeneratedExplanation] = useState<string | null>(null);
  const [aiExporting, setAiExporting] = useState<"csv" | "json" | null>(null);
  ...
  const handleGenerateSQL = async (prompt: string) => {
    ...
    setNlGeneratedExplanation(null);
    try {
      ...
      setActiveTab("manual");
      setNlGeneratedSql(response.data.sql);
      setNlGeneratedExplanation(response.data.explanation);
      message.success("SQL generated! Execute it or export directly below.");
    } catch (error: any) {
  ...
  /** AI-assisted export: execute generated SQL and download file in one step. */
  const handleAiExport = async (format: "csv" | "json") => {
    if (!selectedDatabase || !nlGeneratedSql) return;

    setAiExporting(format);
    try {
      const response = await apiClient.post(
        `/api/v1/dbs/${selectedDatabase}/query-export`,
        { sql: nlGeneratedSql, format },
        { responseType: "blob" }
      );
      ...
      const filename =
        filenameMatch?.[1] ||
        `${selectedDatabase}_${new Date().toISOString().replace(/[:.]/g, "-").slice(0, -5)}.${format}`;
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#F4EFEA" }}>
      ...
      >
        ...
        {/* AI Assistant Export Suggestion - shown after NL query generates SQL */}
        {nlGeneratedSql && (
          <Card
            style={{
              borderWidth: 2,
          ...
          >
            <Row gutter={16} align="middle">
              <Col flex="none">
                <RobotOutlined
                  style={{ fontSize: 28, color: "#16AA98", marginRight: 12 }}
                />
              </Col>
              <Col flex="auto">
                <Typography.Text strong style={{ fontSize: 14, color: "#16AA98" }}>
                  AI 助手
                </Typography.Text>
                <div style={{ marginTop: 4 }}>
                  <Typography.Text style={{ fontSize: 13 }}>
                    需要将这次查询结果导出为 <strong>CSV</strong> 或 <strong>JSON</strong> 文件吗？<br />
                  </Typography.Text>
                  <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                    AI 已生成 SQL：{nlGeneratedSql.length > 80
                      ? nlGeneratedSql.slice(0, 80) + "..."
                      : nlGeneratedSql}
                  </Typography.Text>
                </div>
                {nlGeneratedExplanation && (
                  <Alert
                    message={nlGeneratedExplanation}
                    type="info"
                    showIcon
              ...
              <Col flex="none">
                <Space size={8} direction="vertical" align="end">
                  <Button
                    icon={<FileTextOutlined />}
                    onClick={() => handleAiExport("csv")}
                    loading={aiExporting === "csv"}
                    style={{ fontWeight: 700, height: 36 }}
                  >
                    导出 CSV
                  </Button>
                  <Button
                    icon={<FileTextOutlined />}
                    onClick={() => handleAiExport("json")}
                    loading={aiExporting === "json"}
                    style={{ fontWeight: 700, height: 36 }}
                  >
                    导出 JSON
                  </Button>
