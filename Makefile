.PHONY: help install dev backend frontend test lint format clean db-migrate db-upgrade db-downgrade export-sql export-nl export-list
...
# ---------------------------------------------------------------------------
# Usage:
#   make export-sql DB=mydb SQL="SELECT * FROM users" FORMAT=csv
#   make export-nl DB=mydb PROMPT="查询所有活跃用户" FORMAT=json
#   make export-list
# ---------------------------------------------------------------------------

export-sql: ## Export SQL query result (DB=mydb SQL="SELECT ..." FORMAT=csv|json OUTPUT=file.csv)
	@if [ -z "$(DB)" ] || [ -z "$(SQL)" ]; then \
		echo "$(YELLOW)Usage: make export-sql DB=<name> SQL=\"SELECT ...\" FORMAT=csv|json OUTPUT=$(NC)"; \
		exit 1; \
	fi
	@echo "$(BLUE)Executing SQL and exporting as $(FORMAT)...$(NC)"
	cd $(BACKEND_DIR) && $(UV) run python scripts/db_export_cli.py \
		--db "$(DB)" \
		--sql "$(SQL)" \
		--format $(or $(FORMAT),csv) \
		$(if $(OUTPUT),--output "$(OUTPUT)",)

export-nl: ## Natural language query + export (DB=mydb PROMPT="描述" FORMAT=csv|json OUTPUT=file.csv)
	@if [ -z "$(DB)" ] || [ -z "$(PROMPT)" ]; then \
		echo "$(YELLOW)Usage: make export-nl DB=<name> PROMPT=\"描述\" FORMAT=csv|json OUTPUT=$(NC)"; \
		exit 1; \
	fi
	@echo "$(BLUE)Converting NL to SQL, executing and exporting as $(FORMAT)...$(NC)"
	cd $(BACKEND_DIR) && $(UV) run python scripts/db_export_cli.py \
		--db "$(DB)" \
		--prompt "$(PROMPT)" \
		--format $(or $(FORMAT),csv) \
...
export-list: ## List registered database connections
	@echo "$(BLUE)Registered databases:$(NC)"
	cd $(BACKEND_DIR) && $(UV) run python scripts/db_export_cli.py --list-dbs
